...
def store(invalid=False):
    if request.method == 'POST':
        ...
        result = requests.post(settings.STORE_URL + 'queue', json={"id": mongo_id})
...
def fetch_responses_from_store(invalid=False):
    ...
    request_url = '{}{}'.format(settings.STORE_URL, 'responses')
    if invalid:
        invalid = True
        request_url = '{}{}'.format(settings.STORE_URL, 'invalid-responses')
